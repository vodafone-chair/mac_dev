...
{
public:
  UeInfo(uint8_t ueAddress): m_ueAddress(ueAddress), m_timeEnqueued(time(NULL)), m_maxQueuingTime(20)
  {
  }

  UeInfo(uint8_t ueAddress, time_t maxQueuingTime): m_ueAddress(ueAddress), m_timeEnqueued(time(NULL)), m_maxQueuingTime(maxQueuingTime)
  {
  }

  UeInfo(uint8_t ueAddress, time_t time, time_t expire): m_ueAddress(ueAddress), m_timeEnqueued(time), m_maxQueuingTime(expire)
  {
  }
...
public:
  ...
  static void ConvertStringToChar (char* charOut, std::string stringIn); // converts a string to a char array
  static int GenerateRandomNumber (uint64_t maxNum);                     // generates a random number between 0 and maxNum
  static bool IsStringEqual (std::string string1, std::string string2);  // compares string1 with string2 if it is equal
  static bool IsStringEqual (char* char1, char* char2);                  // compares char with char2 if it is equal
  static bool IsStringIncluded(char* charTotal, char* charIncluded);     // checks if charIncluded is in charTotal
  static uint16_t ComputeCrc(const char* data, uint32_t len);            // compute the CRC of a packet
...
public:
  ...
  static uint8_t GetZeroAddress();
  static uint8_t GetBaseStationAddress();

  friend std::ostream & operator<< (std::ostream &out, const Header &header);

}; /* Header */

std::ostream & operator<< (std::ostream &out, const Header &header); // to print Header with std::cout
...
public:
  ...
  uint32_t Serialize (char* outBuffer);               // add header to the payload
  static PacketData Deserialize (char* inBuffer, uint32_t lenHeadAndPayl); // remove header from the payload

  friend std::ostream & operator<< (std::ostream &out, const PacketData &packet);

}; /* PacketData */

std::ostream & operator<< (std::ostream &out, const PacketData &packet); // to print PacketData with std::cout
